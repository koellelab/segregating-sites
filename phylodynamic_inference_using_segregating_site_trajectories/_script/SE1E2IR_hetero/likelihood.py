# ------------------
## likelihood.py (Salje_hetero; SE1E2IR model)
# ------------------
import copy
import numpy as np
import sys
from SE1E2IR_hetero import smc
# ------------------
def calculate (particles, params, imported_data, config):
    ...
    for this_win_idx in range(len(sim_windows)):
        ...
        if (np.isnan(data['S_in_win'][this_win_idx])).all():    # no data exist for any of clades
            ...
            print(this_win_idx, "(", data['data_flag'][this_win_idx, :], ")", ":", sum(w_matrix[this_win_idx, :]), sum_likelihood_passage, np.log(sum_likelihood_passage), log_likelihood)
