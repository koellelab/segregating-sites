...
def run (params, imported_data, config):
    ...
    while True:
        ...
        for idx in range(len(sim_window)):
            ...
            if this_time_end % 5 == 0:
                print(this_time_end)
            # simulate one step
            params['model_nextdt'](particle, params, this_time_start, this_time_end)


            ## check when each genotype
            print(f'{this_time_end:5.2f} {np.where(particle.genotype_count[:, 0] > 0)[0][:5]}')
            #print(f'{this_time_end:5.2f} {particle.genotype_count[:5, 1]}')

            # record the genotype recovered during this step
            sample_genotype     = particle.genotype_count[:, params['idx_sampling']] > 0
            particle.recovered  = pd.concat([particle.recovered,
