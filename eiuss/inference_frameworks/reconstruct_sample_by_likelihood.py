...
def run(params, imported_data, config):
    ...
    if True:
        ML = df_mean_llk.mean_logL.nanmax()
        threshold = ML - scipy.stats.chi2.ppf(1 - 0.05, df=len(params["operators"]))
        df_mean_llk = df_mean_llk[df_mean_llk.mean_logL >= threshold]
        sampled_idx = params['rng'].choice(len(df_mean_llk.mean_logL), size=params["n_reps"], replace=True)
