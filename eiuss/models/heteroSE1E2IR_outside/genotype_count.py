...
def infection_event_mutate(count_event, statevar_curr, genotype_count, clade_idx, genotype_info, n_genotype, n_site, param_mu, rng, rng2):
    ...
    for i in range(infection_src_idx.size):
        ...
        if (int(n_ind_total) != src_count.sum()):
            print("src_count:", src_count, s_idx)
        ...
        if (n_genotype_unchanging > 0) and (n_ind_to_pick > 0):
            ...
            try:
                ...
                mutation_count = rng2.multinomial(n=n_ind_to_pick, pvals=param_mu)
            except:
                print("src_count:", src_count, s_idx, n_genotype_unchanging, n_ind_to_pick, n_ind_total,statevar_curr, count_event)
                raise Exception(f'chosen_genotype')
