...
import utils
from models.heteroSE1E2IR_outside import gillespie
from models.heteroSE1E2IR_outside import genotype_count as genotype_count_
##################################################################
...
def update(particle, param, this_win_start, this_win_end):
    ...
    # setup for outside reservoir
    if not hasattr(particle, "infected_from_outside"):
        particle.infected_from_outside = []
        #particle.statevar      [:, compartments['O_l']+2] = np.exp(param['r'] * (particle.statevar[:, 0] - param['t_e'])) * (1 - param['p_H'])
        #particle.statevar      [:, compartments['O_h']+2] = np.exp(param['r'] * (particle.statevar[:, 0] - param['t_e'])) * param['p_H']
        #particle.genotype_count[0, compartments['O_l']] = 1
        #particle.genotype_count[0, compartments['O_h']] = 1
    ...
    for idx in range(len(t_range)):
        ...
        #print(statevar_curr.tolist())
        ## ----------------------------
        ## 0. update genotype_count for outside reservoir
        statevar_curr[compartments['O_l']+2] = np.exp(param['r'] * (statevar_curr[0] - param['t_e'])) * (1 - param['p_H'])
        statevar_curr[compartments['O_h']+2] = np.exp(param['r'] * (statevar_curr[0] - param['t_e'])) * param['p_H']
        particle.genotype_count[0, compartments['O_l']] = statevar_curr[compartments['O_l']+2]
        particle.genotype_count[0, compartments['O_h']] = statevar_curr[compartments['O_h']+2]

        #print(statevar_curr[compartments['O_l']+2], particle.genotype_count[0, compartments['O_l']], statevar_curr[compartments['O_h']+2], particle.genotype_count[0, compartments['O_h']])
        ...
        ## ----------------------------
        ## 4. safety check
        check_statevar_ = statevar_next[2:param['idx_sampling']+2].astype("int")  ## except t, S, R
        check_genotype_ = particle.genotype_count[:, :param['idx_sampling']].sum(axis=0)  ## excetp R
        ...
        particle.infected_from_outside.append([t_now_end, event_count_infection[:-2].sum()])
        t_idx += 1
...
#    for i in range(len(particle.infected_from_outside)):
#        print(f'{particle.infected_from_outside[i][0]: 5.3f} |  {particle.infected_from_outside[i][1]: 5.3f} ')
    return
